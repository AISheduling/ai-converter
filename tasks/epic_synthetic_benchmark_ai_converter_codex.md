...
- **TASK-Bench-01** — канонические модели, deterministic generator, deterministic renderers, persistence.
- **TASK-Bench-02** — heterogeneous subobjects и synthetic drift generation с переиспользованием `ai_converter.drift`.
- **TASK-Bench-03** — LLM-assisted template generation и cache/validation поверх `ai_converter.llm`.
- **TASK-Bench-04** — расширение существующего `ai_converter.evaluation` для synthetic benchmark и stage-wise metrics.
- **TASK-Bench-05** — grouped reporting, examples/runner, experiment UX и end-to-end smoke coverage.

Рекомендуемый порядок: `TASK-Bench-01 -> TASK-Bench-02 -> TASK-Bench-03 -> TASK-Bench-04 -> TASK-Bench-05`.

---

# TASK-Bench-01. Канонические модели, deterministic generator, deterministic renderers и сохранение bundles
...
# TASK-Bench-02. Генерация heterogeneous subobjects и framework для drift-сценариев
...
# TASK-Bench-03. LLM-assisted генерация разнообразных `L0TemplateSpec`, cache и validation gates
...
# TASK-Bench-04. Benchmark harness, адаптер к `ai-convertor`, `N` запусков и сбор stage-wise metrics
...
# TASK-Bench-05. CLI, итоговые метрики, boxplots, отчеты и end-to-end smoke coverage
